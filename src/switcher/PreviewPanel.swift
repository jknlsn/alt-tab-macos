...
class PreviewPanel: NSPanel {
    ...
    static func show(_ id: CGWindowID, _ preview: CALayerContents, _ position: CGPoint, _ size: CGSize) {
        ...
        if id != currentId || !Self.shared.isVisible {
            ...
            currentId = id
            Self.shared.order(.below, relativeTo: App.currentPanel?.windowNumber ?? TilesPanel.shared.windowNumber)
            ...
            Self.shared.level = NSWindow.Level(rawValue: App.currentPanel?.level.rawValue ?? TilesPanel.shared.level.rawValue - 1)
        }
    }
